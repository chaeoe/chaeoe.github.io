...
특정 인물이나 사건, 주제를 드러내는 어휘가 아니라서
소설의 내용이나 특성을 파악하는 데 도움이 되지 않는다고 판단하여 제외했다.  
`one`은 막연한 지시어이고, `would`, `could`는 조동사로서 문법적 기능을 수행한다.
`said`는 인물 간의 대화를 인용하는 상황에서 자주 등장하는 어휘일 뿐이며,
`even`은 주위의 내용을 강조하는 부사이다.

### 두 작품의 단어 빈도가 들려주는 이야기
- **공통으로 도드라지는 단어**: 두 작품 모두 `saw`, `found` 같은 어휘가 상위에 보인다.
이는 두 소설이 화자의 **시선과 발견**을 통해 공포감이나 긴장감을 쌓아 가는 서술 방식을 공유하고 있음을 시사한다.
또 `man`이 공통으로 보이는데, 이는 *Frankenstein*의 피조물과 *Dracula*의 흡혈귀가 모두
**인간과 비인간의 경계**와 관련된 존재라는 점과 연결 지어 볼 수 있다.
- **한 작품에만 도드라지는 단어**와 그것이 시사하는 작품의 특성:
*Frankenstein*에서는 `father`, `life`, `first` 같은 단어가 두드러지는데,
이는 이 소설의 주제인 창조자와 피조물의 관계, 생명의 창조와 잘 어우러진다.
